...
# --- 2. SAFE & BALANCED DATA GENERATOR (CRASH-PROOF) ---
@st.cache_data(ttl=3600)
def compile_dataset_matrix():
    records = []
    
    # 100% Safe fallback block: Directly populate records for ALL 20 categories uniformly
    np.random.seed(42)
    sample_logs = [
    ...
    # Strictly generates rows distributed evenly over all 20 options
    for i in range(4000):
        ...
        # Plain inline conditional logic to avoid nested syntax errors
        if score > 0.15:
            sentiment = 'Positive'
        elif score < -0.15:
            sentiment = 'Negative'
...
st.sidebar.write("Architecture Pipeline v20.0 • Final Syntax Clear")
st.sidebar.write("---")

st.sidebar.subheader("🎛️ Filter Matrix Configurations")

# Hard force all 20 categories selected by default on page load
