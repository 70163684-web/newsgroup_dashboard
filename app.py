...
# --- 2. STABLE DATA EXTRACTION ENGINE ---
...
def compile_dataset_matrix():
                    ...
                    if member.isfile() and ("train" in member.name or "test" in member.name):
                        ...
                        if len(parts) >= 3:
                            ...
                            try:
                                ...
                                # CRITICAL FIX: Standardized clean conditional parsing blocks
                                if score > 0.15:
                                    sentiment = 'Positive'
                                elif score < -0.15:
                                    sentiment = 'Negative'
    ...
    # Safe Fallback Dataset Simulation Layer
    simulated_rows = []
    np.random.seed(42)
...
# --- 3. SIDEBAR Hub CONTROL INTERFACE ---
st.sidebar.title("🔮 Navigation Hub")
st.sidebar.write("Architecture Pipeline v11.0 • Stable Execution")
st.sidebar.write("---")
...
# All target categories selected automatically by default
...
with tab_words:
    ...
    if not working_df.empty and len(working_df) > 0:
        ...
        if frequent_tokens:
            ...
            fig_tokens = px.bar(token_df, x='Frequency', y='
